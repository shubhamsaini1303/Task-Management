...
const TaskManagement = () => {
  ...
  const createTask = async () => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/v1/task/create`, newTask, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Task created successfully!");
      setNewTask({ title: "", description: "", dueDate: "", status: "Pending" });
  ...
  const updateTask = async () => {
    if (!editingTask) return;
    try {
      await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/v1/task/update/${editingTask}`, editedTask, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Task updated successfully!");
      setShowPopup(false);
  ...
  const deleteTask = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/v1/task/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Task deleted successfully!");
      fetchTasks();
  ...
  return (
    <>
      ...
      <div className="p-4 md:p-8">
        <h1 className="text-center text-2xl font-bold">Task Management</h1>
        <div className="mt-4">
          <input type="text" placeholder="Title" value={newTask.title} onChange={(e) => setNewTask({ ...newTask, title: e.target.value })} className="border p-2 mr-2" />
          <input type="text" placeholder="Description" value={newTask.description} onChange={(e) => setNewTask({ ...newTask, description: e.target.value })} className="border p-2 mr-2" />
          <input type="date" value={newTask.dueDate} onChange={(e) => setNewTask({ ...newTask, dueDate: e.target.value })} className="border p-2 mr-2" />
          <button onClick={createTask} className="bg-blue-500 text-white px-4 py-2 rounded">Create Task</button>
        </div>
        <div className="mt-6">
          {tasks.length > 0 ? (
            tasks.map((task, index) => (
              <div key={task._id} className="border p-4 my-2 rounded shadow-lg">
                <h2 className="font-bold">{index + 1}. {task.title}</h2>
                <p>{task.description}</p>
                <p className="text-gray-600">Due: {new Date(task.dueDate).toLocaleDateString()}</p>
                <div className="flex gap-2 mt-2">
                  <button onClick={() => { setEditingTask(task._id); setEditedTask(task); setShowPopup(true); }} className="bg-yellow-500 text-white px-3 py-1 rounded">Edit</button>
        ...
        {showPopup && (
          <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-gray-500 bg-opacity-50">
            <div className="bg-white p-6 rounded-md">
              ...
              <input type="text" value={editedTask.title} onChange={(e) => setEditedTask({ ...editedTask, title: e.target.value })} className="border p-2 w-full mb-2" />
              <button onClick={updateTask} className="bg-green-500 text-white px-4 py-2 rounded">Save</button>
            </div>
          </div>
        )}
      </div>
